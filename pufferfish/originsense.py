...
#           = P(predict positive | actual positive) = Recall = P(B^A)/P(A) = intersection of predicted and actual positives / actual positives
...
#           = P(predict neg | actual positive) = P(Bc^A)/P(A) = P(predict neg ^ actual pos)/ P(actual pos)
FNR = 1 - SN
scores['FNR'] = FNR

#7.	PPV = P(A|B) = Prob(Ori_bp | Peak_bp) = numOriBpInsidePeakCoords / Peak_bp = % Peak_bp labeled as origin
#       = P(actual pos | predict pos) = Precision = P(A^B)/P(B) 
...
#       = P( actual neg | pred pos ) = P(Ac^B)/P(B) = % of pred pos that are false pos
FDR = 1 - PPV
scores['FDR'] = FDR

#9.	SP = P(Bc|Ac) = Prob(Non_Peak_bp | Non_Ori_bp) = numNonPeakBpInsideNonOriCoords / Non_Origin_bp = % Non_Ori_bp NOT labeled as Peak_bp
#           = P(pred neg | actual neg ) = P(Bc^Ac)/P(Ac) = % actual neg predicted neg
